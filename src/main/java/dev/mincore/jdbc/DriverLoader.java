 ...
 *
 * <ol>
 *   <li>System property {@code mincore.jdbc.driver} (exact file path).
...
public final class DriverLoader {
  private static final org.slf4j.Logger LOG = LoggerFactory.getLogger("mincore");
  private static final String PROP_OVERRIDE = "mincore.jdbc.driver";
  ...
  public static void tryLoadMariaDbDriver() {
    ...
    if (jar == null) {
      LOG.error(
          "(mincore) MariaDB JDBC driver not found. Place mariadb-java-client-<version>.jar in mods/lib/mariadb/ "
    ...
    try {
      ...
      LOG.info("(mincore) JDBC driver loaded from {} (file: {})", jar.getParent(), jar.getName());
    } catch (Throwable t) {
      LOG.error("(mincore) failed to load JDBC driver from {}", jar.getPath(), t);
    }
  }
  ...
  private static File findDriverJar() {
    ...
    if (override != null && !override.isBlank()) {
      ...
      LOG.warn("(mincore) {} set but file not found: {}", PROP_OVERRIDE, override);
    }
