...
public final class Migrations {
  ...
  public static void apply(Services services) {
    ...
    try (Connection c = services.database().borrowConnection();
      ...
      for (String sql : ddl) {
        try {
          ...
          allSucceeded = false;
          LOG.warn(
              "(holarki) migration statement failed; continuing. cause={} sql=\n{}",
              e.getMessage(),
              sql);
        }
      }
    ...
    if (!allSucceeded) {
      LOG.warn("(holarki) migrations completed with errors; schema version unchanged");
      return;
    }
  ...
  private static void recordSchemaVersion(Services services) {
    try (Connection c = services.database().borrowConnection();
      ...
      ps.executeUpdate();
      c.commit();
      LOG.info("(holarki) schema version recorded: {}", CURRENT_VERSION);
    } catch (SQLException e) {
