...
| Timezone services | Enabled | `modules.timezone.enabled`; GeoIP via `modules.timezone.autoDetect.enabled` | Respect the player override flag—APIs still exist but `/timezone set` may be unavailable. Server operators must set `core.time.display.allowPlayerOverride = true` before enabling GeoIP auto-detect or they will hit the validator error `core.time.display.autoDetect requires allowPlayerOverride=true`. |
