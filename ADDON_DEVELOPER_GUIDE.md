...
| Timezone services | Enabled | `modules.timezone.enabled`; GeoIP via `modules.timezone.autoDetect.enabled` | Respect the player override flag—APIs still exist but `/timezone set` may be unavailable. Server operators only need to set `core.time.display.allowPlayerOverride = true` before enabling GeoIP auto-detect; the module now turns on `core.time.display.autoDetect` automatically (leave the legacy toggle in place only when migrating older configs). |
